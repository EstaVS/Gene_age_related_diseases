...
tumor_samples <- brain_data[, brain_data$sample_type == "Primary Tumor"]

# Subset normal samples
normal_samples <- brain_data[, brain_data$sample_type == "Solid Tissue Normal"]
...
gene_info <- as.data.frame(rowData(brain_data))
...
ggplot(pca_df, aes(x = PC1, y = PC2)) +
  ...
  theme_minimal() +
  labs(title = "PCA of Brain Cancer Gene Expression Data",
...
pca_plot <- ggplot(pca_df, aes(x = PC1, y = PC2, color = as.factor(phylostrata))) +
  geom_point(alpha = 0.7, size = 2) +
  labs(
    title = "PCA of Differential Expression results for Brain Cancer samples",
    color = "Phylostrata"
  ) +
  labs(color = "Phylostrata") +
  theme_classic()

ggsave("brain_cancer_pca.png", plot = pca_plot, width = 8, height = 6, dpi = 300)
...
pca_plot_2 <- ggplot(pca_df_subset, aes(x = PC1, y = PC2, color = as.factor(phylostrata))) +
  geom_point(alpha = 0.7, size = 2) +
  labs(
    title = "PCA of Differential Expression results for Brain Cancer samples - Only phylostrata 1-4",
    color = "Phylostrata"
  ) +
  labs(color = "Phylostrata") +
  theme_classic()

ggsave("brain_cancer_pca_subset.png", plot = pca_plot_2, width = 8, height = 6, dpi = 300)
...
pca_plot_3 <- ggplot(pca_df_subset_2, aes(x = PC1, y = PC2, color = as.factor(phylostrata))) +
  geom_point(alpha = 0.7, size = 2) +
  labs(
    title = "PCA of Differential Expression results for Brain Cancer samples - Only phylostrata 10-11",
    color = "Phylostrata"
  ) +
  labs(color = "Phylostrata") +
  theme_classic()

ggsave("brain_cancer_pca_subset_2.png", plot = pca_plot_3, width = 8, height = 6, dpi = 300)
...
violin_plot <- ggplot(all_genes_filtered_clean, aes(x = as.factor(Phylostrata), y = log2FoldChange, fill = as.factor(Phylostrata))) +
  ...
  theme_classic() +
  labs(
    title = "Brain Cancer Log2FoldChange Results by Phylostrata",
    x = "Phylostrata",
    y = "log2(Fold Change)",
    fill = "Phylostrata"
  ) +
...
ggsave("brain_cancer_violin.png", plot = violin_plot, width = 8, height = 6, dpi = 300)
